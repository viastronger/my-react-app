...
class app extends Component {
    ...
    setDataToDom(data, w, h) {
        ...
        this.pointArray.forEach((item, i) => {
            ...
            children.push(
                <TweenOne className="point-wrapper" key={item.x * r} style={{ left: item.x, top: item.y }}>
                    <TweenOne
                        className="point"
                        style={{
                            width: r,
    ...
    gatherData = () => {
        this.setState((prev) => {
            const children = prev.children && prev.children.map((item) => React.cloneElement(item, {
                animation: {
                    x: 0,
                    y: 0,
                    opacity: 1,
    ...
    disperseData = () => {
        ...
        this.setState((prev) => {
            const children = prev.children.map((item) => React.cloneElement(item, {
                animation: {
    ...
    render() {
        return (
            <div className="logo-gather-demo-wrapper" ref={(c) => { this.componentDom = c }}>
                <canvas id="canvas" />
                <TweenOne
